// Load in dependencies
var http = require('http'),
    spawn = require('child_process').spawn,
    util = require('util'),
    colors = require('fn-colors');

// Export a module to spawn servers
// TODO: Inherit from EventEmitter
// TODO: Instead of logging inline, emit events
// TODO: At the end of constructor set up event listeners
function ListenSpawn(cmd, args, options) {
  // TODO: Fallback args
  // Fallback options
  options = options || {};

  // Define a method to start/restart the child
  var child;
  function restartChild() {
    // If there is a previous process running, kill it
    if (child) {
      // Notify that we are killing the child
      util.log.green('[listen-spawn] Killing existing child');

      // Kill previous child
      child.kill();
      child.on('exit', cb);
    } else {
      cb();
    }

    // Start a new process
    function cb() {
      // Notify that the command is starting
      util.log.green('[listen-spawn] Starting new process -- ' + cmd + ' ' + args.join(' '));

      // Start the process
      child = spawn(cmd, args, {stdio: [0, 1, 2]});

      // If the child leaves, notify them of the exit code and whatnot
      child.on('exit', function (code) {
        if (code === 0) {
          util.log.green('[listen-spawn] App exited cleanly');
        } else {
          util.log.green('[listen-spawn] App left with exit code ' + code);
        }

        // Nullify the child @_@
        child = null;
      });
    }
  }

  // Start a new server
  var server = http.createServer(function startListenSpawnServer (req, res) {
        // Notify that a request has been received
        util.log.green('[listen-spawn] Request received');

        // Restart the child
        restartChild();

        // Close the connection with a 204
        res.writeHead(204);
        res.end();
      });

  // Begin listening
  var port = options.port || 7670;
  server.listen(port);

  // Notify the server is listening
  util.log('[listen-spawn] Listening at http://localhost:' + port + '/');

  // Start the child
  // TODO: Expose as a method
  restartChild();

  // When the process is terminated, kill the child
  // TODO: Expose as a method
  // TODO: This is terrible for multiple instances
  process.on('SIGTERM', function () {
    // Kill the child, callback when we are good to leave
    if (child) {
      child.kill();
      child.on('exit', cb);
    } else {
      cb();
    }

    // Leave the process for good
    function cb() {
      process.exit(0);
    }
  });
}

// Export ListenSpawn
module.exports = ListenSpawn;